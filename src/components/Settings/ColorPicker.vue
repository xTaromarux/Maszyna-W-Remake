<template>
  <div class="cp-root" @dragstart.prevent>
    ...
    <div class="cp-wheel-wrap" :style="{ width: size + 'px', height: size + 'px' }">
      ...
      <div class="cp-indicator" :style="{ left: indicator.x + 'px', top: indicator.y + 'px', background: hex }"></div>
    ...
    <div class="cp-readout">
      ...
      <div class="cp-text">
        ...
        <div class="cp-line">
          rgb({{ Math.round(rgb.r * brightnessLocal) }}, {{ Math.round(rgb.g * brightnessLocal) }},
          {{ Math.round(rgb.b * brightnessLocal) }})
        </div>
        <div class="cp-line">hsv({{ hsv.h.toFixed(0) }}, {{ Math.round(hsv.s * 100) }}%, {{ Math.round(hsv.v * 100) }}%)</div>
      </div>
    </div>
  </div>
</template>
...
watch(
  () => props.brightness,
  (v) => {
    brightnessLocal.value = clamp01(v);
  }
);
