...
export default {
  ...
  methods: {
    _refreshHighlight() {
      if (this.codeCompiled && Array.isArray(this.compiledProgram) && this.compiledProgram.length) {
        const max = this.compiledCode?.length ?? this.compiledProgram.length
        this.activeLine = Math.max(0, Math.min(this.activeInstrIndex, max - 1))
      }
    },
    ...
    executeLine() {
      ...
      if (this.codeCompiled && Array.isArray(this.compiledProgram) && this.compiledProgram.length > 0) {
        ...
        // Honoruj 'wel' jako skok tylko w trybie legacy (bez compiledProgram)
        // (w trybie strukturalnym skoki robi meta.kind === 'JUMP' / 'CJUMP')
        if (resolvedPhase.wel === true && !this._branchJoin && (!this.compiledProgram || this.compiledProgram.length === 0)) {
        ...
        if (this.activePhaseIndex >= instr.phases.length) {
          ...
          // Jeśli właśnie wykonaliśmy instrukcję w obszarze ISR – wróć z przerwania
          if (this.activeInstrIndex >= (this.interrupts.vectorBase|0) && this.isrStack.length) {
            const ctx = this.isrStack.pop();
            this.activeInstrIndex = ctx.pc;
            this.activePhaseIndex = ctx.phaseIdx;
            this._refreshHighlight();
            this.addLog('[INT] powrót z ISR', 'system');
            return;
          }
