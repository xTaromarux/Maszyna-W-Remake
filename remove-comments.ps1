# Skrypt do usuwania komentarzy z plików JavaScript/TypeScript/Vue
param(
    [string]$Path = "src",
    [switch]$DryRun = $false
)
...
function Remove-Comments {
    ...
    # Usuń komentarze // (pojedyncze linie)
    $content = $content -replace '(?m)^\s*//.*$', ''
    
    # Usuń komentarze inline // ale zachowaj URL-e
...
# Znajdź wszystkie pliki JS/TS/Vue w katalogu src
$files = Get-ChildItem -Path $Path -Recurse -Include "*.js", "*.ts", "*.vue" | Where-Object { $_.Name -notlike "*.min.*" }

$processedCount = 0
$modifiedCount = 0

Write-Host "Znaleziono $($files.Count) plików do przetworzenia..." -ForegroundColor Cyan

foreach ($file in $files) {
    $processedCount++
    $modified = Remove-Comments -FilePath $file.FullName -DryRun $DryRun
    if ($modified) {
        $modifiedCount++
    }
}
